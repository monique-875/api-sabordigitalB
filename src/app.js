...
const queryAsync = (sql, values = [] ) => {// async  tranforma a função em assincrona
    return new Promise((resolve, reject) => { //retornar o resultado da promesa
...
app.get('/', (req, res) => {// Testar se a api está ativa
...
app.get('/produtos', async (req, res) => {
    ...
    } catch (erro){//executado apenas se um erro ocorrerno  no try
...
app.get('/produtos/:id', async (req, res) => {
   try {
        ...
        if(!id || isNaN(id)){// isNan é not a number
...
app.post('/produtos', async (req, res) => {
    try {
        ...
        const resultado = await queryAsync('INSERT INTO produto SET ?',[novoProduto])//tentativa de salvar no banco de dados
...
app.put('/produtos/:id', async (req,res) =>{
    try {
        ...
        if(produtoExistente.length === 0){
            return res.status(404).json({// 404 quando nao localiza uma informação
                sucesso: false,
        ...
        if(Object.keys(produtoAtualizado).length === 0){// analiza todos os objetos presentes no java
            return res.status(400).json({
                sucesso: false,
